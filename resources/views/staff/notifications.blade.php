{{-- resources/views/staff/notifications.blade.php --}}
...
<div class="card">
    ...
    @php
        ...
        $icons = ['trip_approved'=>'✅','trip_rejected'=>'❌','rate_driver'=>'★','trip_cancelled'=>'🚫'];
    ...
    <div style="padding:14px 20px;border-bottom:1px solid var(--border);display:flex;gap:14px;
        ...
        @if(isset($data['url']))
            @if(isset($data['trip_id']) && str_contains($data['type'],'rate'))
            <button class="btn btn-amber btn-sm"
                onclick="openRatingModal({{ $data['trip_id'] }},'Driver','TRP')">★ Rate</button>
            @else
            <a href="{{ $data['url'] }}" class="btn btn-ghost btn-sm">View →</a>
            @endif
        @endif
    </div>
    @empty
