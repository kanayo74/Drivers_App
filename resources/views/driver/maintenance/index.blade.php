			...
			<form method="POST" action="{{ route('driver.maintenance.store') }}">
				...
				@if(in_array('role:admin', request()->route()->middleware()))
					<div class="form-group">
						...
						<input name="driver_name" class="form-input" placeholder="Automatically Generated" disabled/>
						<input name="driver_id" class="form-select" required hidden/>
					</div>
				@endif
				<div class="form-group">
					<label class="form-label">Vehicle</label>
					<select name="vehicle_id" class="form-select" id="vehicle_id" onchange="driver(this)" required>
						@if(!empty($vehicles))
							<option value="" disabled selected>Select vehicle…</option>
							@foreach($vehicles as $v)
								<option data-driver="{{ $v->assignedDriver->name }}" data-driver-id="{{ $v->assignedDriver->id }}" value="{{ $v->id }}">{{ $v->plate_number }} — {{ $v->make }} {{ $v->model }}</option>
							@endforeach
						@else
							<option disabled selected>No Vehicles Found.</option>
						@endif
					</select>
				</div>
				<div class="form-group">
...
@push('scripts')
	<script>
		 ...
		 */
		function driver(select) {
			const selectedOption = select.querySelector('option:checked');
			const driverNameInput = document.querySelector('#maintenance-modal input[name="driver_name"]');
			const driverIDInput = document.querySelector('#maintenance-modal input[name="driver_id"]');
			driverNameInput.setAttribute('value', selectedOption.dataset.driver)
			driverIDInput.setAttribute('value', selectedOption.dataset.driverId)
		}
